...
function objectProjection2(arrayOfObjects) {
  var cucu = function(item) {
    return ({
      id: item.id,
      title: item.title
    });
  }
  return arrayOfObjects.myMap(cucu); // finish this expression!
}
...
function goodMovieId(arrayOfObjects) {
  ...
  return arrayOfObjects.myFilter(caca).myMap(cucu);
